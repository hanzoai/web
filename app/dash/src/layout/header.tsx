            ...
            <PopoverContent className='bg-level-0 mr-8 !px-0 text-sm text-primary font-light border-level-1'>
              <div className="grid gap-2">
                ...
                <div className='flex flex-col'>
                  <div className='h-8 flex flex-row justify-between truncate rounded-sm hover:bg-level-1 hover:cursor-pointer items-center mx-2 px-2' onClick={() => setOpenCommandMenu(true)}>Dashboard</div>
                  <div className='h-8 flex flex-row justify-between truncate rounded-sm hover:bg-level-1 hover:cursor-pointer items-center mx-2 px-2' onClick={() => setOpenCommandMenu(true)}>Dashboard</div>
                  <div className='h-8 flex flex-row justify-between truncate rounded-sm hover:bg-level-1 hover:cursor-pointer items-center mx-2 px-2' onClick={() => setOpenCommandMenu(true)}>Dashboard</div>
                </div>
                <Separator className='bg-level-1'/>
                <div className='h-8 flex flex-row justify-between truncate rounded-sm hover:bg-level-1 hover:cursor-pointer items-center mx-2 px-2' onClick={() => setOpenCommandMenu(true)}>
                  <p>Command Menu</p>
                ...
                <Separator className='bg-level-1' />
                <div className='px-2'>
                  <Button variant="primary" onClick={() => handleLogOut} className='w-full'>Logout</Button>
                </div>
              </div>
            </PopoverContent>
          </Popover>
